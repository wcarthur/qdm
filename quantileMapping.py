...
def plotQuantiles(refdata, futdata, scenario, start, end, plotpath, title):
    ...
    for i, (m, refdf) in enumerate(tempref.items()):
        ...
        srefql, srefq, srefqu = quantileCI(srefdata)
        sfutql, sfutq, sfutqu = quantileCI(sfutdata)
        ax[i].scatter(srefq, sfutq, alpha=0.5)
        fill_no_autoscale(srefql, srefqu, sfutqu, sfutql, ax=ax[i], alpha=0.7, linewidth=0)
...
def plotQuantileDelta(refdata, futdata, scenario, start, end, plotpath, title):
    ...
    for i, (m, refdf) in enumerate(tempref.items()):
        ...
        srefdata = refdf.pdiff.values
        sfutdata = futdf.pdiff.values
        deltal, deltam, deltau = deltaCI(srefdata, sfutdata)

        ax[i].fill_between(x, deltal, deltau, alpha=0.7)
        ax[i].scatter(x, deltam, alpha=0.5)
...
def fill_no_autoscale(xlower, xupper, ylower, yupper, ax, **kwargs):
    """
...
def deltaCI(ref, fut):
    """
    Calculate confidence intervals of the delta values

    We do this by resampling the reference and future data values, calculating
    the quantiles for each and then the ratio of quantiles. Then for each
    quantile 0 < q < 1, calculate the 5th 50th and 95th percentile.
    """
    nsamples = 10**4
    x = np.linspace(0, 1, 100)
    delta = np.zeros((nsamples, len(x)))
    for n in range(nsamples):
        refq = np.quantile(np.random.choice(ref, 1000), x)
        futq = np.quantile(np.random.choice(fut, 1000), x)
        delta[n,:] = futq / refq
    return np.quantile(delta, [0.05, 0.5, 0.95], axis=0)

def quantileCI(data):
    """
    Calculate confidence interval on quantiles using bootstrap resampling

    Assumes we want the 5th, 50th and 95th percentile values
    """
    nsamples = 10**4
    x = np.linspace(0, 1, 100)
    quantiles = np.zeros((nsamples, len(x)))
    for n in range(nsamples):
        quantiles[n, :] = np.quantile(np.random.choice(data, 1000), x)
    return np.quantile(quantiles, [0.05, 0.5, 0.95], axis=0)

if __name__ == '__main__':

    path = r"./data/source"
    plotpath = "./data/figures"
    regex = r'all_tracks_(.+)_(rcp\d+)\.dat'
    obsfile = "./data/obs/ibtracs.since1980.list.v04r00.csv"
    ...
    refgrpparams.to_csv(pjoin(outputPath, "refgrpfitparameters.csv"), index=False)
    futgrpparams.to_csv(pjoin(outputPath, "futgrpfitparameters.csv"), index=False)
    brefgrpparams.to_csv(pjoin(outputPath, "brefgrpfitparameters.csv"), index=False)
    bfutgrpparams.to_csv(pjoin(outputPath, "bfutgrpfitparameters.csv"), index=False)
